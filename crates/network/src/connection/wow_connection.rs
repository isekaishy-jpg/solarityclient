...
impl WorldConnection {
        ...
        S: AsyncRead + AsyncWrite + Unpin + Send,
    {
        solarity_profiling::profile_await!(
            "network.connection.wow_connection.authenticate",
            async {
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send,
{
    pub(super) async fn advance(mut self) -> Result<WorldAuthProgress<S>, WorldAuthError> {
        solarity_profiling::profile_await!("network.connection.wow_connection.advance", async {
            let message = ServerOpcodeMessage::tokio_read_encrypted(
                &mut self.stream,
                self.crypto.decrypter(),
            )
            .await
            .map_err(|error| WorldAuthError::Decode {
