...
impl CloudImage {
    ...
    pub(super) fn create(
        ...
        let (image, memory) = unsafe { allocator.create_image(&info, &allocation) }
            .map_err(|source| VulkanError::operation("create cloud image", source))?;
        self.image = image;
        ...
        self.view = unsafe { device.create_image_view(&view, None) }
            .map_err(|source| VulkanError::operation("create cloud image view", source))?;
        Ok(())
    }
    ...
    /// Records a full replacement before the frame's first cloud sample.
    pub(super) fn upload(
        &self,
        device: &Device,
        ...
        let barrier = vk::ImageMemoryBarrier2::default()
            ...
            .image(self.image)
            .subresource_range(color_range());
        // SAFETY: Synchronization2 orders the complete upload before cloud sampling.
        unsafe {
            device.cmd_pipeline_barrier2(
                command,
                &vk::DependencyInfo::default().image_memory_barriers(&[barrier]),
...
/// Describes the one complete color mip shared by all procedural cloud images.
