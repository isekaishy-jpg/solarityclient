//! Borrowed descriptor ABI, shader modules, and fixed particle pipeline state.
...
use crate::{M2BlendFactor, M2MaterialState, M2ParticleRenderVertex, M2ParticleSpirvProgram};

/// Pipeline layout borrowing the renderer's compatible M2 descriptor layouts.
#[derive(Default)]
pub(super) struct M2ParticlePipelineLayout {
    handle: vk::PipelineLayout,
    scene_set: vk::DescriptorSetLayout,
    texture_set: vk::DescriptorSetLayout,
}

impl M2ParticlePipelineLayout {
    pub(super) fn ensure_created(
        ...
        if self.handle != vk::PipelineLayout::null() {
            if self.scene_set != scene_set || self.texture_set != texture_set {
                return Err(VulkanError::M2ParticleDescriptorLayoutChanged);
            }
            return Ok(());
        }
        ...
        self.handle = unsafe { device.create_pipeline_layout(&info, None) }.map_err(|source| {
            VulkanError::operation("create M2 particle pipeline layout", source)
        })?;
...
pub(super) fn create_pipeline(
    ...
    let bindings = [vk::VertexInputBindingDescription::default()
        .binding(0)
        .stride(M2ParticleRenderVertex::BYTE_SIZE as u32)
        .input_rate(vk::VertexInputRate::VERTEX)];
    let attributes = [
        vertex_attribute(0, vk::Format::R32G32B32_SFLOAT, 0),
        vertex_attribute(1, vk::Format::R32G32B32_SFLOAT, 12),
        vertex_attribute(2, vk::Format::B8G8R8A8_UNORM, 24),
        vertex_attribute(3, vk::Format::R32G32_SFLOAT, 28),
    ];
    ...
    unsafe { device.create_graphics_pipelines(pipeline_cache, &[info], None) }
        .map_err(|(_partial, source)| {
            VulkanError::operation("create M2 particle graphics pipeline", source)
        })?
        .into_iter()
        .next()
        .ok_or_else(|| {
            VulkanError::operation(
                "create M2 particle graphics pipeline",
    ...
    fn create(
        ...
        let vertex =
            unsafe { device.create_shader_module(&vertex_info, None) }.map_err(|source| {
                VulkanError::operation("create M2 particle vertex shader module", source)
            })?;
        ...
        let fragment = match unsafe { device.create_shader_module(&fragment_info, None) } {
            ...
            Err(source) => {
                ...
                return Err(VulkanError::operation(
                    "create M2 particle fragment shader module",
                    source,
                ));
            }
        };
