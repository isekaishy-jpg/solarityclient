//! Upload-ready ordinary head-billboard preparation.
...
/// Recovered branches that do not use the ordinary camera-facing head quad.
const UNSUPPORTED_HEAD_FLAGS: u32 = 0x0000_0400 | 0x0000_4000 | 0x0020_0000 | 0x0100_0000;
...
/// Dynamic ordinary-head vertices and triangle indices for one emitter.
...
impl M2ParticleMeshPlan {
    /// Builds camera-facing head quads from one placement-local live set.
    ///
    /// # Errors
    ///
    ...
    pub fn prepare_head(
        ...
        let unsupported = emitter.flags() & UNSUPPORTED_HEAD_FLAGS;
        if unsupported != 0 {
        ...
        for particle in particles {
            ...
            let up = camera.up();
            let cell = appearance.head_texture_cell();
            let cell_origin =
                Vec2::new((cell & (columns - 1)) as f32, (cell / columns) as f32) * cell_size;
            let base_vertex = u32::try_from(vertices.len())
                .map_err(|_source| M2ParticleMeshPlanError::VertexCount)?;
            let mut color = appearance.color();
            color.w *= alpha_multiplier;
            for (corner, coordinate) in CORNERS.into_iter().zip(CELL_COORDINATES) {
                let scaled = corner * appearance.scale();
                let rotated = Vec2::new(
                    scaled.x * cosine - scaled.y * sine,
                    scaled.x * sine + scaled.y * cosine,
                );
                let position = particle.position() + right * rotated.x + up * rotated.y;
                vertices.push(M2ParticleRenderVertex {
                    position: position.to_array(),
                    normal: (-camera.forward()).to_array(),
                    color_bgra: pack_bgra(color.to_array()),
    ...
    /// Returns the two triangles emitted for every ordinary head quad.
    #[must_use]
...
/// One emitter cannot enter the ordinary head-billboard mesh path.
...
pub enum M2ParticleMeshPlanError {
    ...
    #[error("M2 geometry particle cannot use an ordinary head billboard")]
    GeometryParticle,
    /// One or more flags select a specialized recovered render path.
    #[error("M2 particle head behavior flags 0x{0:08X} are not implemented")]
    BehaviorFlags(u32),
