...
void main() {
    ...
    // Stock Color_T1/CDiffuse_T1 VS c30 applies depth, then the exponent,
    // then the upper clamp. The explicit view plane also covers Model cameras
    // whose parallel projection has a constant clip W.
    float eye_depth = dot(scene.view_depth_plane, vec4(in_position, 1.0));
    float linear_visibility = max((scene.fog_parameters.y - eye_depth) / fog_range, 0.0);
    out_fog_visibility = min(pow(linear_visibility, scene.fog_parameters.w), 1.0);
}
