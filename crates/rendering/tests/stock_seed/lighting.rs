...
/// A receiver appends its exterior contribution without rewriting scene sources.
#[test]
fn wotlk_sunlight_merger_accepts_separate_ordered_source_banks() -> Result<(), &'static str> {
    let scene = [M2DirectionalLight::new(
        Vec3::X,
        Vec3::splat(0.1),
        Vec3::splat(0.4),
    )];
    let exterior = M2DirectionalLight::new(-Vec3::X, Vec3::splat(0.1), Vec3::splat(0.4));
    let sunlight = merge_wotlk_directional_lights(scene.iter().chain(std::iter::once(&exterior)))
        .ok_or("scene and exterior sources did not produce sunlight")?;
    assert_eq!(sunlight.direction(), Vec3::Z);
    assert_eq!(sunlight.ambient(), Vec3::splat(0.4));
    assert_eq!(sunlight.diffuse(), Vec3::ZERO);
    assert!(merge_wotlk_directional_lights(std::iter::empty()).is_none());
    Ok(())
}
