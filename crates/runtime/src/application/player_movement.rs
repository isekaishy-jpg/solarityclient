...
pub enum RuntimePlayerMovementError {
    ...
    /// A sampled interval cannot resolve finite collision travel.
    #[error(transparent)]
    IntervalDrive(#[from] solarity_systems::MovementIntervalDriveError),
...
impl LocalMovement {
    ...
    fn snapshot(&self) -> (WorldTransform, WorldMovementState) {
        ...
        let mut movement = WorldMovementState::new(
        ...
        if let Some(spline) = self.published.1.spline() {
            movement = movement
