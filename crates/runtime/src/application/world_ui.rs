...
pub enum RuntimeWorldUiError {
    /// The server has not supplied the action-bar image required by FrameXML.
    #[error("active player has no authoritative action-button image")]
...
impl RuntimeWorldUi {
    ...
    pub(super) fn prepare(
        ...
        world.set_realm_time(
            UiRealmTime::new((half_minutes / 120) as u8, ((half_minutes % 120) / 2) as u8)
                .map_err(RuntimeWorldUiError::from)?,
        );
        let action_buttons = action_buttons.ok_or(RuntimeWorldUiError::MissingActionButtons)?;
        let slots = action_buttons
            .slots()
            .ok_or(RuntimeWorldUiError::MissingActionButtons)?;
