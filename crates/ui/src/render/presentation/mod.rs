...
use resolve::{PresentationResolver, ResolvedObjects};
...
pub struct UiPresentationPlan {
    ...
    disabled_texture_parents: Vec<(usize, Option<usize>)>,
}
...
impl UiPresentationPlan {
    ...
    pub(crate) fn resolve(
        ...
        let resolver = PresentationResolver {
            ...
            geometry,
            backdrops,
            disabled_texture_owners: &disabled_texture_owners,
        };
        let mut output = ResolvedObjects::default();
        for object_index in 0..live.objects().len() {
            resolver.append(object_index, &mut output);
        }
        let ResolvedObjects {
            mut keyed,
            mut models,
        ...
        Self {
            ...
            disabled_texture_parents: live
                .objects()
                .iter()
                .enumerate()
                .filter(|(_, object)| object.role == UiObjectRole::DisabledTexture)
