//! External stock-compatibility test modules for this crate.

#[path = "stock_seed/support/mod.rs"]
mod support;

#[path = "stock_seed/sound_admission.rs"]
mod sound_admission;
#[path = "stock_seed/world_scale.rs"]
mod world_scale;

#[path = "stock_seed/addon.rs"]
mod addon;
#[path = "stock_seed/addon_loading.rs"]
mod addon_loading;
#[path = "stock_seed/animation.rs"]
mod animation;
#[path = "stock_seed/binding.rs"]
mod binding;
#[path = "stock_seed/binding/assignments.rs"]
mod binding_assignments;
#[path = "stock_seed/combat_log.rs"]
mod combat_log;
#[path = "stock_seed/event.rs"]
mod event;
#[path = "stock_seed/feature/action_bar.rs"]
mod feature_action_bar;
#[path = "stock_seed/feature/battlefield.rs"]
mod feature_battlefield;
#[path = "stock_seed/feature/battlenet.rs"]
mod feature_battlenet;
#[path = "stock_seed/feature/chat.rs"]
mod feature_chat;
#[path = "stock_seed/feature/commentator.rs"]
mod feature_commentator;
#[path = "stock_seed/feature/container.rs"]
mod feature_container;
#[path = "stock_seed/feature/dress_up.rs"]
mod feature_dress_up;
#[path = "stock_seed/feature/group_finder.rs"]
mod feature_group_finder;
#[path = "stock_seed/feature/guild_bank.rs"]
mod feature_guild_bank;
#[path = "stock_seed/feature/health_bar.rs"]
mod feature_health_bar;
#[path = "stock_seed/feature/item_text.rs"]
mod feature_item_text;
#[path = "stock_seed/feature/loot.rs"]
mod feature_loot;
#[path = "stock_seed/feature/merchant.rs"]
mod feature_merchant;
#[path = "stock_seed/feature/minimap.rs"]
mod feature_minimap;
#[path = "stock_seed/feature/name_plate.rs"]
mod feature_name_plate;
#[path = "stock_seed/feature/paper_doll.rs"]
mod feature_paper_doll;
#[path = "stock_seed/feature/party.rs"]
mod feature_party;
#[path = "stock_seed/feature/portrait.rs"]
mod feature_portrait;
#[path = "stock_seed/feature/quest.rs"]
mod feature_quest;
#[path = "stock_seed/feature/runtime_boundary.rs"]
mod feature_runtime_boundary;
#[path = "stock_seed/feature/spell_book.rs"]
mod feature_spell_book;
#[path = "stock_seed/feature/taxi_map.rs"]
mod feature_taxi_map;
#[path = "stock_seed/feature/tooltip.rs"]
mod feature_tooltip;
#[path = "stock_seed/feature/trade.rs"]
mod feature_trade;
#[path = "stock_seed/feature/trade_skill.rs"]
mod feature_trade_skill;
#[path = "stock_seed/feature/trainer.rs"]
mod feature_trainer;
#[path = "stock_seed/font.rs"]
mod font;
#[path = "stock_seed/frame.rs"]
mod frame;
#[path = "stock_seed/glue.rs"]
mod glue;
#[path = "stock_seed/glue/character.rs"]
mod glue_character;
#[path = "stock_seed/glue/creation.rs"]
mod glue_creation;
#[path = "stock_seed/glue/model.rs"]
mod glue_model;
#[path = "stock_seed/glue/text_refresh.rs"]
mod glue_text_refresh;
#[path = "stock_seed/glue/tooltip.rs"]
mod glue_tooltip;
#[path = "stock_seed/input.rs"]
mod input;
#[path = "stock_seed/notification.rs"]
mod notification;
#[path = "stock_seed/region.rs"]
mod region;
#[path = "stock_seed/render.rs"]
mod render;
#[path = "stock_seed/script.rs"]
mod script;
#[path = "stock_seed/widget/button.rs"]
mod widget_button;
#[path = "stock_seed/widget/check_box.rs"]
mod widget_check_box;
#[path = "stock_seed/widget/color_select.rs"]
mod widget_color_select;
#[path = "stock_seed/widget/edit_box.rs"]
mod widget_edit_box;
#[path = "stock_seed/widget/edit_box_layout.rs"]
mod widget_edit_box_layout;
#[path = "stock_seed/widget/html.rs"]
mod widget_html;
#[path = "stock_seed/widget/hyperlink.rs"]
mod widget_hyperlink;
#[path = "stock_seed/widget/message.rs"]
mod widget_message;
#[path = "stock_seed/widget/model.rs"]
mod widget_model;
#[path = "stock_seed/widget/movie.rs"]
mod widget_movie;
#[path = "stock_seed/widget/scroll_frame.rs"]
mod widget_scroll_frame;
#[path = "stock_seed/widget/slider.rs"]
mod widget_slider;
#[path = "stock_seed/widget/status_bar.rs"]
mod widget_status_bar;
#[path = "stock_seed/widget/texture.rs"]
mod widget_texture;
#[path = "stock_seed/world.rs"]
mod world;
#[path = "stock_seed/xml.rs"]
mod xml;
