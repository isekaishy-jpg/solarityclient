...
local childLevel = OwnerScroller:GetFrameLevel()
Owner:Raise()
assert(Owner:GetFrameLevel() > ownerLevel)
assert(OwnerScroller:GetFrameLevel() - Owner:GetFrameLevel() == childLevel - ownerLevel)
...
<Frame name="First"><Attributes>
  ...
  assert(format('%2$s hits %1$s for %3$04d (%%)', 'target', 'caster', 87) == 'caster hits target for 0087 (%)')
  assert(string.format('%2$s %s %1$s %s', 'one', 'two', 'three') == 'two three one two')
  assert(format('%10$s/%1$s', 1, 2, 3, 4, 5, 6, 7, 8, 9, 'ten') == 'ten/1')
  assert(format('%1$.2f/%1$.1f', 1.26) == '1.26/1.3')
  assert(format('%0$s') == '%0$s')
  assert(format('price $%d, %%2$s', 7) == 'price $7, %2$s')
  assert(not pcall(format, '%3$s', 'one') and not pcall(format, '%100$s', 'one'))
  ...
  assert(GetCVar("timeMgrAlarmEnabled") == "0")
  assert(GetCVar("lastTalkedToGM") == "")
  assert(GetCVar("autoQuestWatch") == "1")
  assert(GetCVarDefault("gxVSync") == "1")
  assert(GetCVarMin("farclip") == 177 and GetCVarMax("farclip") == 1277)
  assert(GetCVarDefault("horizonFarclipScale") == "4.0")
  assert(GetCVar("horizonFarclipScale") == "4.0")
  assert(GetCVarMin("horizonFarclipScale") == 3 and GetCVarMax("horizonFarclipScale") == 6)
  SetCVar("horizonFarclipScale", "5.75")
  assert(GetCVar("horizonFarclipScale") == "5.75")
  SetCVar("horizonFarclipScale", "4.0")
  ...
  assert(GetCVarDefault("equipmentManager") == "0")
  assert(GetCVarDefault("previewTalents") == "0")
  assert(GetCVarDefault("showTutorials") == "1")
  assert(GetCVarDefault("showGameTips") == "1")
  assert(GetCVarDefault("UberTooltips") == "1")
  assert(GetCVarDefault("scriptErrors") == "0")
  assert(GetCVarDefault("trackerSorting") == "0")
  assert(GetCVarDefault("trackerFilter") == "7")
  assert(GetCVarDefault("nameplateShowFriends") == "0")
  assert(GetCVarDefault("nameplateShowEnemies") == "0")
  ...
  local addonLoaded, addonReason = LoadAddOn("Missing")
  assert(addonLoaded == nil and addonReason == "MISSING")
...
<Button name="FontButton" text="PLAYER"><ButtonText name="$parentText"/><NormalFont style="GlueFontTest"/><Frames><Frame name="$parentOwned" parentKey="owned"/></Frames><Scripts><OnLoad>
  ...
  local reparentedShows, reparentedHides = 0, 0
  reparented:SetScript("OnShow", function() reparentedShows = reparentedShows + 1 end)
  reparented:SetScript("OnHide", function() reparentedHides = reparentedHides + 1 end)
  newParent:Hide()
  assert(not reparented:IsVisible() and reparentedHides == 1)
  newParent:Show()
  assert(reparented:IsVisible() and reparentedShows == 1)
  ...
  FontLabel:SetVertexColor(1, 1, 1)
  local vr, vg, vb, va = FontLabel:GetVertexColor()
  assert(vr == 1 and vg == 1 and vb == 1 and va == 1)
  ...
  assert(GradientTexture:SetDesaturated() == 1)
  GradientTexture:SetDesaturated(nil)
  assert(GradientTexture:IsDesaturated() == nil)
  assert(GradientTexture:SetDesaturated(true) == 1)
  ...
  check:SetChecked("0")
  assert(check:GetChecked() == false)
  assert(type(check:GetChecked()) == "boolean")
  ...
  tooltip:Hide()
  assert(tooltip:GetOwner() == nil and not tooltip:IsOwned(self))
  tooltip:SetOwner(self, "INVALID_ANCHOR")
  assert(tooltip:GetPoint() == "BOTTOMRIGHT")
  assert(not pcall(function() tooltip:SetOwner(tooltip) end))
