...
discovery, synchronous asset preparation or shader compilation. Making the inner
...
| 1, admission stalls | Compile shadow shaders inside the supposedly precompiled GPU publication path | A cache miss synchronously invokes shaderc and driver pipeline creation on the presentation owner |
...
[`synchronize_remote_players`](../../crates/runtime/src/application/player_coordinator.rs#L2003)
...
[`visible_unit_guids`](../../crates/ecs/src/world/state.rs#L319) walks the entire
...
[`synchronize_replicated_animations`](../../crates/runtime/src/application/player_coordinator.rs#L1490),
...
[`refresh_dependency_regions`](../../crates/ui/src/region/geometry.rs#L242)
...
## 3. Precompiled publication can still compile a shader synchronously

[`M2PipelineRegistry::prepare_precompiled`](../../crates/rendering/src/device/vulkan_m2_pipeline/registry.rs#L173)
checks the retained handle, creates the ordinary driver pipeline, then, for a
non-shadow permutation, constructs `M2SpirvCompiler` and compiles
`permutation.with_primary_shadow()` before creating a second pipeline. The
worker-prepared bytecode therefore does not cover all programs publication needs.

[`prepare_gpu_source_with_plan`](../../crates/runtime/src/application/terrain_frame/m2.rs#L4408)
visits all material draws and can request ordinary and runtime-fade variants.
[`synchronize_static_scenes`](../../crates/runtime/src/application/terrain_frame/m2/streaming.rs#L154)
can publish several previously unseen sources in the same call. Warm cache hits
are cheap; a new material during movement can take this expensive path. This is
a hitch mechanism, not evidence of shader compilation on every steady frame.

Required contract: a prepared source contains every required shader variant;
presentation performs no source compilation. Driver creation must have explicit
admission rather than admitting every miss in a source/tile transaction. The
existing Glue pipeline-admission mechanism is useful precedent, but its one-item
limit alone is not a hard time bound on an individual driver call.

## 4. Whole-generation admission still crosses the frame boundary

[`load_remote_player`](../../crates/runtime/src/application/player_coordinator.rs#L2182)
...
[`TerrainFrame::synchronize_tiles`](../../crates/runtime/src/application/terrain_frame/streaming.rs#L47)
prepares every arriving tile's terrain/liquids, then all new M2 and WMO membership,
before returning. It is called from
[`service_terrain_streaming`](../../crates/runtime/src/application/client_services/world_camera.rs#L63).
...
[`GroundDetailWorld::prepare`](../../crates/runtime/src/application/terrain_frame/ground_detail.rs#L74):
...
[`RuntimeRemoteMovement::service`](../../crates/runtime/src/application/player_movement/remote/service.rs#L69)
queries submerged liquid for every eligible remote owner after advancing it.
[`unit_submerged_liquid`](../../crates/runtime/src/application/terrain_coordinator/movement/liquid.rs#L120)
performs unit registration; its
[`probe_registration_roots`](../../crates/runtime/src/application/terrain_coordinator/movement/registration.rs#L191)
...
[`resolved_world_camera`](../../crates/runtime/src/application/client_services/world_camera.rs#L11)
...
[`recompose_with_overrides`](../../crates/runtime/src/application/terrain_frame/m2.rs#L2903)
for each admitted model. The
[`pose composer`](../../crates/rendering/src/model/m2_animation/pose.rs#L243)
...
[`WorldFrameSlot::write`](../../crates/rendering/src/device/vulkan_world_frame/resource.rs#L421)
...
The general
[`refresh_targeted_objects`](../../crates/ui/src/glue/c_glue_mgr.rs#L1023)
...
[`RuntimeGameplayCoordinator::service_with_game_objects`](../../crates/runtime/src/application/gameplay_coordinator.rs#L440)
...
dependencies for recurring work; complete worker shader preparation and bounded
