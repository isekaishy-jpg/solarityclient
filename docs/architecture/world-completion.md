...
| World and terrain lighting | Terrain diffuse/specular lighting, native texture density/filtering, weighted/unlit layer composition and animated layer offsets are implemented with native comparisons. The terrain point-light shader and single-chunk query are connected; native paired-chunk query bounds remain. Complete the remaining lighting consumers and shader/material variants, including specialized entity callbacks. The user reports continuing differences from stock; compare the combined presentation at matching camera, time, and settings. |
