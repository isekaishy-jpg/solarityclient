...
| World and terrain lighting | Terrain diffuse/specular lighting, native texture density/filtering, weighted/unlit layer composition and animated layer offsets are implemented with native comparisons. Terrain point lights include native material-based neighboring-chunk pairing and shared query bounds, with decoded terrain and GPU comparisons. Complete the remaining lighting consumers and shader/material variants, including specialized entity callbacks. The user reports continuing differences from stock; compare the combined presentation at matching camera, time, and settings. |
| Environment shaders | Audit native material selection and the environment shader families exercised by actual world assets. ProcWater is a confirmed unsupported family, but its referenced reflection map and first height frame are absent from the installed archives; use in the reported scene is unproven. Ordinary water/magma have native comparisons. Per-model liquid classification, transparent pass selection and intersection clipping have native classification, attachment regression, controlled GPU coverage and an installed-archive surface-crossing replay. The combined populated-world comparison remains open. The native liquid shaders have no shadow-map receivers. See [liquid presentation](liquid-rendering.md). Continue distinguishing missing shaders from incorrect inputs or unconnected consumers. |
| Buildings and portals | Surface, liquid, attached-doodad and sky admission/fog consumers are integrated with native and controlled GPU coverage. Ordinary units and M2 game objects retain their registered owner's first accepted fog bank; meshes and attached particle scenes share that bank, with native cases and static/moving-building runtime pixels. Validate their combined live presentation while entering, leaving and moving through buildings. |
| Shadows | Primary and higher-quality environment maps, unit/scenery/WMO casters, cached publication and cascade transitions, and terrain/model/ground-detail receivers are integrated. Native and Vulkan comparisons, installed Durotar captures, and all three quality travel runs pass. Initial asynchronous scenery publication still needs reconciliation with native cache refresh. Hardware comparison sampling, quality-zero behavior, and exceptional registrations remain under investigation; see [shadows](world-shadows.md). |
...
| Particles, spells, and aura visuals | Ordinary particle owner-opacity blending/alpha testing and ribbon birth opacity follow their distinct native submission paths. Ribbon color, culling, grouped material order and retained depth fog now have native shader/state, Vulkan pixel and runtime coverage, including fog inherited from preceding M2, particle and WMO submissions. Complete the remaining specialized particle paths and general spell/aura visual consumers and lifecycles. Validate attachment, ordering, visibility, and retirement together. Track checks blocked by missing spell or other gameplay implementation explicitly. |
