...
eight. The native small/POT allocation rules remain further work, as does
combined populated-world appearance. The separate Special owner is covered below.
...
## Procedural Special screen composition

Constructor `7EA5F0` registers `ffxSpecial`, creates a fixed 256x256 procedural
noise texture, and retains a 256x128 history target. Callback `4F7020` clears
history and supplies the declaration's parameters to `7E9010`. The first word
is packed ARGB material color; the second is a signed integer multiplied by
the native float approximation of `1/255` for alpha decay; the third is a
signed integer multiplied by `0.01` for desaturation strength. Selection starts
a three-second countdown, preserving the noise-row cursor.

`7E8E40` fills white RGB and procedural alpha through `985580`, `9854D0` and
`9852A0`. The implementation reproduces the wrapping polynomial, five octaves,
smoothed neighbors and original float stores. All 262,144 generated bytes
match the pinned executable. Cargo generates and embeds this texture during
the build, so activation performs no procedural CPU generation. GPU upload
uses retained staging and the graphics queue without a host wait.

`7E92A0` seeds a noise strip at bottom-origin Y=3..4 and solid color at Y=0..3.
Noise sampling advances by one row per effect draw, including zero-delta
draws; row 256 is sampled with clamping before wrapping to zero. The Vulkan
seed pass combines those strips with an exact copy of the remaining history.
The propagation shader preserves the archive's tap order with offsets
`(-1,1), (0,1), (1,1), (0,2)`, averages RGBA, then subtracts decay from alpha.
Every history pass writes linear RGBA8, retaining native quantization.

The original graph samples and writes the same history texture. Tracing
`8C1890`, `6A3070`, `6A2D80`, `6A4900` and `6A5940` found direct texture/target
binding rather than an implicit copy. The Direct3D oracle compares that graph
with a separate destination over 813 frames, including reselection and image
reset; every history byte agrees. Vulkan uses two explicit images with barriers
covering prior reads and writes. These images belong to the renderer, not a
presentation slot, and remain valid across disabled draws and other effects.
Resource recreation clears images while retaining the process-owned cursor
and countdown.

`7E8410`/`7E8380`/`7E8600` supply a 33x33 polar mesh. Its radius and normalized
acos angle sample history, while a second coordinate samples the world scene.
The shader first desaturates toward equal-weight gray, then whitens, then
blends toward history RGB by history alpha. `7E9670` publishes both ramp
constants before decrementing the countdown. The implementation preserves
the original intermediate precision, including the tiny negative initial
values that the shader clamps to zero. Procedural vertex indices reproduce
the native triangle order and half-pixel placement without per-frame uploads.

Runtime selection, final world presentation and offline benchmark presentation
now use this owner. Live integer `ffx` and `ffxSpecial` settings gate advancement
and drawing. Unknown effect types retain the selected owner and its parameters;
reselection resets the ramp/history even while drawing is disabled. UI draws
after composition and retains its original colors.

`special_screen_oracle.py` captures noise, mesh, 45 ramp samples and 260 seed
draws from the original executable. `special_screen_shader_oracle.py` executes
the unchanged archive propagation and composition pixel shaders with native
mesh/constants and equivalent fixed-function seed input. The retained
sequences use the Kvaldir mist, Realm of Shadows and Ebon Hold transition
parameters. All 48 Vulkan captures at 64x64, 65x61 and 80x48 differ by at most
one byte value per channel. Additional checks exercise disabled draws, another
effect, resource replacement, opaque UI and restoration of the unchanged scene.
Runtime tests cover settings, unknown types, reselection and retained cursors.
The UI CVar catalog registers the default enabled value and passes all 36
native integer-parsing cases for `ffxSpecial` through the live script observer.

Three optimized real-archive replays completed 1,260 frames each on map 1 at
`(1300, -4530, 50)`, camera distance 25, with a 200-unit outward/return route.
Every frame selected Special. The benchmark's `special_desaturation` field
reached 0.4, 0.6 and 1.0 for declaration IDs 41, 123 and 201 respectively.
Inspected settled captures show the white Kvaldir mist, partial desaturation
and shadow tint, and the black/grayscale transition. The minimap and other UI
retain their colors. All three logs contain no renderer warnings or errors.
These captured runs validate the connected presentation, not performance.

Four uncaptured replays then ran on/off/off/on with 2,400 frames per phase,
declaration 41, and otherwise identical profiles. Each enabled run selected
Special in all 16,800 frames; each disabled run selected no screen effect.
Stationary and settled phases retained 49 tiles and identical detail/shadow
draw counts. No compiler or test process ran during the comparison; the local
auth/world servers remained running throughout.

On the GTX 1070 at 1280x720, enabled stationary medians were 2.76890 and
2.85035 ms (about 351–361 FPS), versus disabled medians of 2.77125 and
2.79855 ms. This spread does not isolate a precise incremental effect cost.
Enabled stationary p99 ranged from 3.4942 to 4.6820 ms, with maxima of
20.7887 and 40.7772 ms. Pointer maxima ranged from 33.8 to 44.0 ms across
the four runs; initial streaming maxima ranged from 47.5 to 52.4 ms.
The second disabled run also reached 41.1 ms after travel. These measurements
do not establish stall removal or the aspirational 1,200 FPS rate.

The full workspace passed 1,209 tests with 23 ignored. The subsequent CVar
registration correction passed the focused script/bootstrap regression, and
workspace Clippy passed again with all targets/features and warnings denied.

These checks establish the Special owner on normalized textures with linear,
